...
import { delArticle } from "@/apis/article";
const { Option } = Select;
...
const Article = () => {
  ...
  const { channelList, article, setParams, params } = useChanel();
  ...
  const onFinish = (formData) => {
    // 1. 准备参数
    const { channel_id, date, status } = formData;
    const reqData = {
      status,
      channel_id,
  ...
  return (
    <div>
      ...
      >
        <Form
          initialValues={{ status: 0, channel_id: "推荐" }}
          onFinish={onFinish}
        >
